...
import { createSessionAndGo, openFolderSession } from "../../lib/session/actions";
import { normalizeDirectory } from "../../lib/session/folders";
...
import { FolderDialog } from "../sessions/FolderDialog";
...
export function AppShell(props: { children: JSX.Element }) {
  ...
  async function newSession(directory?: string) {
    await createSessionAndGo(navigate, invalidateSessions, directory);
  }

  /** Native picker on desktop; shared dialog on web (no silent no-op). */
  async function newSessionInFolder() {
    if (isDesktop()) {
      const directory = await pickFolder();
      if (directory) await openFolderSession(navigate, invalidateSessions, directory);
      return;
    }
    uiStore.setFolderDialogOpen(true);
  }

  /** Route a `rudra://` URL or folder path to a session (dedups by folder). */
  async function openTarget(raw: string | null | undefined) {
    const target = parseOpenTarget(raw);
    if (!target) return;
    if (target.kind === "session") {
      const id = target.id;
      if (sessionStore.state.sessions.some((s) => s.id === id)) {
        navigate(`/s/${id}`);
      } else {
        try {
      ...
      const normalized = normalizeDirectory(target.path);
      if (normalized) await openFolderSession(navigate, invalidateSessions, normalized);
    }
  }
