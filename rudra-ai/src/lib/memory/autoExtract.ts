...
import { routeTask } from "../routing/tiers";
...
export async function runExtraction(sessionId: string): Promise<number> {
  ...
  try {
    ...
    // Phase 3 routing: extraction is mechanical — prefer a cheap model when one
    // is installed. Any failure falls back to the server default (no behavior change).
    let extractModel: { providerID: string; modelID: string } | undefined;
    try {
      const providers = await adapter.listProviders();
      extractModel = routeTask("extract", providers.providers ?? []).model;
    } catch {
